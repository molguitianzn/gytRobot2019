...
const UpperOutputs walkUpper = {M_PI/4, -2*M_PI/3, M_PI/4, -2*M_PI/3, 0, -M_PI/4};

mutex head_mutex, task_mutex, gdata_mutex;

std::vector<simulation::BodyAngles> LoadAct(std::string name)
{
    ifstream ifs(name);
    if(!ifs)
    {
        ROS_ERROR("can not find: %s", name.c_str());
        return {};
    }
    std::vector<simulation::BodyAngles> ret;
...
UniRobot::UniRobot(string name, std::string actdir): Robot()
{
    mReset=false;
    mName = name;
    mActdir = actdir;
    ...
    mAngleSubscriber = mNode->subscribe("/bodyangles", 10, &UniRobot::AnglesUpdate, this);
    ...
    mResetServer = mNode->advertiseService(name+"/reset", &UniRobot::SetReset, this);
}

void UniRobot::AnglesUpdate(const simulation::BodyAngles::ConstPtr &p)
{
    if(p->id==0){
        mBodyAngles.clear();
    }
    mBodyAngles.push_back(*(p.get()));
    if(p->total == mBodyAngles.size())
    {
        mAct=true;
    }
}
...
bool UniRobot::SetReset(std_srvs::SetBool::Request &req, std_srvs::SetBool::Response &res)
{
    mReset = static_cast<bool>(req.data);
}
...
void UniRobot::run(){
    ...
    while(ros::ok())
    {
        if(mAct)
        {
            Rhoban::IKWalkOutputs lastOuts;
            UpperOutputs lastUOuts;
            for(auto angles:mBodyAngles)
            {
                ...
                {
                    mLOuts.left_ankle_pitch = lastOuts.left_ankle_pitch + i*(angles.left_ankle_pitch-lastOuts.left_ankle_pitch)/angles.time;
                    mLOuts.left_ankle_roll = lastOuts.left_ankle_roll + i*(angles.left_ankle_roll-lastOuts.left_ankle_roll)/angles.time;
                    mLOuts.left_hip_pitch = lastOuts.left_hip_pitch + i*(angles.left_hip_pitch-lastOuts.left_hip_pitch)/angles.time;
                    mLOuts.left_hip_roll = lastOuts.left_hip_roll + i*(angles.left_hip_roll-lastOuts.left_hip_roll)/angles.time;
                    mLOuts.left_hip_yaw = lastOuts.left_hip_yaw + i*(angles.left_hip_yaw-lastOuts.left_hip_yaw)/angles.time;
                    mLOuts.left_knee = lastOuts.left_knee + i*(angles.left_knee-lastOuts.left_knee)/angles.time;
                    mLOuts.right_ankle_pitch = lastOuts.right_ankle_pitch + i*(angles.right_ankle_pitch-lastOuts.right_ankle_pitch)/angles.time;
                    mLOuts.right_ankle_roll = lastOuts.right_ankle_roll + i*(angles.right_ankle_roll-lastOuts.right_ankle_roll)/angles.time;
                    mLOuts.right_hip_pitch = lastOuts.right_hip_pitch + i*(angles.right_hip_pitch-lastOuts.right_hip_pitch)/angles.time;
                    mLOuts.right_hip_roll = lastOuts.right_hip_roll + i*(angles.right_hip_roll-lastOuts.right_hip_roll)/angles.time;
                    mLOuts.right_hip_yaw = lastOuts.right_hip_yaw + i*(angles.right_hip_yaw-lastOuts.right_hip_yaw)/angles.time;
                    mLOuts.right_knee = lastOuts.right_knee + i*(angles.right_knee-lastOuts.right_knee)/angles.time;
                    
                    mUOuts.left_shoulder = lastUOuts.left_shoulder + i*(angles.left_shoulder-lastUOuts.left_shoulder)/angles.time;
                    mUOuts.left_elbow = lastUOuts.left_elbow + i*(angles.left_elbow-lastUOuts.left_elbow)/angles.time;
                    mUOuts.right_shoulder = lastUOuts.right_shoulder + i*(angles.right_shoulder-lastUOuts.right_shoulder)/angles.time;
                    mUOuts.right_elbow = lastUOuts.right_elbow + i*(angles.right_elbow-lastUOuts.right_elbow)/angles.time;
                    myStep();
                }
            }
            mAct=false;
        ...
        if(task.type == simulation::Task::TASK_WALK){
            ...
            mBodyAngles = LoadAct(mActdir+"/left_kick");
            Rhoban::IKWalkOutputs lastOuts;
            mUOuts = walkUpper;
            for(auto angles:mBodyAngles)
            {
